...
new Vue({
  ...
  methods: {
    ...
    toggleAllEmails() {
      this.emailClients.forEach(
        (client) => (client.selected = this.selectAllEmails),
      );
    },

    sendEmail() {
      const selectedClients = this.emailClients.filter(
        (client) => client.selected,
      );
      if (selectedClients.length > 0) {
        this.emailConfirmation = true;
      } else {
        // Show an alert or notification that no clients are selected
        alert(this.$t("noClientsSelected"));
      }
    },

    confirmSendEmail(currentClient) {
      const selectedClients = this.emailClients.filter(
        (client) => client.selected,
      );
      // Implement your email sending logic here
      this.emailConfirmation = false;
      this.emailSend = false; // Close the email modal
    ...
    // Computed property to get the count of selected clients
    selectedClientsCount() {
      return this.emailClients.filter((client) => client.selected).length;
    },

    addEmails() {
      const newEmails = this.emailAddList
        .split(",")
      ...
      this.api
        ...
        .then(async () => {
          alert(this.$t("emailAddedSuccess"));
        })
        .catch((err) => {
          alert(err.message || err.toString());
        });
    ...
    deleteEmails() {
      this.emailClients = this.emailClients.filter(
        (client) => !this.emailsToDelete.includes(client.email),
      );

      this.api
        .deleteClientsEmails({
          clients: this.emailsToDelete,
        })
        .then(async () => {
          this.emailsToDelete = [];

          alert(this.$t("emailDeletedSuccess"));
        })
        .catch((err) => {
          alert(err.message || err.toString());
        });
    ...
    addClientsEmails(emails) {
      this.api
        .addClientsEmails({ clients: emails })
        .catch((err) => alert(err.message || err.toString()))
        .finally(() => this.refresh().catch(console.error));
    },
    deleteClientsEmails(emails) {
      this.api
        .addClientsEmails({ clients: emails })
        .catch((err) => alert(err.message || err.toString()))
        .finally(() => this.refresh().catch(console.error));
    ...
    sendEmailToClients(emailClients) {
      this.api
        .sendEmailToClients({ clients: emailClients })
        .catch((err) => alert(err.message || err.toString()))
        .finally(() => this.refresh().catch(console.error));
    },
